...
def preprocess(data_raw):
  data = np.log(data_raw[~np.any(data_raw == 0, axis=1)])
...
def preprocess_raw(data_raw):
  data = data_raw[~np.any(data_raw == 0, axis=1)]
  #print data
  original = np.zeros((data.shape[0] - 5, 1))
  target = np.zeros((data.shape[0] - 5, 1))
  for i in xrange(data.shape[0] - 5):
    original[i] = data[i + 1, 3]
    target[i] = data[i + 1 + 4, 3]
  return np.concatenate([original, target], axis = 1)
...
#dates = []
#for s in Symbols:
  #dates += [np.array(get_all(('daily/' + s)))[-200:, 0]]

#dates = np.concatenate(np.expand_dims(dates, axis=0), axis=0)
#result_dated = np.concatenate([np.expand_dims(dates, axis=2), result5], axis=2)
...
if os.path.isfile('keras-weights7.nn'):
  print 'Loading Model...'
  model.load_weights('keras-weights7.nn')
...
#print 'Saving Model...'
#model.save_weights('keras-weights7.nn', overwrite=True)
...
data_test_raw = []
for d in data_raw:
  data_test_raw += [np.expand_dims(preprocess_raw(d)[-200:, :], axis = 0)]

data_test_raw = np.concatenate(data_test_raw, axis = 0)
result_raw = data_test_raw[:,:,0] ** (np.squeeze(result) + 1)
print np.sqrt(np.mean((result_raw / data_test_raw[:,:,1] - 1) ** 2))
