...
def preprocess(data_raw):
  ...
  result_target = np.expand_dims(result[1:, 3], axis = 1)
  result = result[:-1, :]
...
Symbols = []
with open('symbols_clean2.csv') as csvfile:
...
data_raw = []
for s in Symbols:
  data_raw += [np.array(get_all(('weekly/' + s)))[:, 1:]]

dates = []
for s in Symbols:
  dates += [np.array(get_all(('weekly/' + s)))[-50:, 0]]

dates = np.concatenate(np.expand_dims(dates, axis=0), axis=0)
result_dated = np.concatenate([np.expand_dims(dates, axis=2), result], axis=2)
...
if os.path.isfile('keras-weights6.nn'):
  print 'Loading Model...'
  model.load_weights('keras-weights6.nn')
...
print 'Saving Model...'
model.save_weights('keras-weights6.nn', overwrite=True)
