...
def preprocess(data_raw):
  ...
  indicators = add_all_indicators(data) + 1e-6
  result = indicators[1:, :-1] / indicators[:-1, :-1] - 1
  result = result[:-4, :]
  result_target = np.zeros((result.shape[0], 1))
  for i in xrange(result.shape[0]):
    result_target[i] = indicators[i + 1 + 4, -1] / indicators[i + 1, -1] - 1
...
model.add(TimeDistributedDense(1, activation='tanh'))
...
print 'Saving Model...'
model.save_weights('keras-weights5.nn', overwrite=True)

print 'Testing Model...'
score = model.evaluate(x_test, y_test, batch_size=batch_size, verbose=1)
print 'Test Score: ', score, np.sqrt(score)
