...
data_raw = fetch_since('JMAT.L', '1980-01-01')

#data_all = add_all_indicators(data_raw)
data_all = preprocess(data_raw)

data = data_all[:-200, :-1]
target = data_all[:-200, -1]

data_test = data_all[-200:, :-1]
target_test = data_all[-200:, -1]
...
x_all, y_all = gen_data2(data, target, 200)
x_test = np.expand_dims(data_test, axis = 0)
y_test = np.expand_dims(np.expand_dims(target_test, axis = 1), axis = 0)
...
model.add(LSTM(1, return_sequences=True, activation='tanh', input_shape=(None, x_all.shape[2])))
...
print 'Saving Model...'
model.save_weights('keras-weights1.nn', overwrite=True)
