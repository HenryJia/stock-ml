...
class OutputRNNCell(rnn_cell.RNNCell):
  """The Output RNN Cell"""
...
def run_model(inputs, targets, time_frames):
  ...
  cell = rnn_cell.LSTMCell(lstm1_units, 5)
  layers.append(rnn_cell.DropoutWrapper(cell, output_keep_prob=0.5))
  cell = rnn_cell.LSTMCell(lstm2_units, lstm1_units)
  layers.append(rnn_cell.DropoutWrapper(cell, output_keep_prob=0.5))
  cell = rnn_cell.LSTMCell(lstm3_units, lstm2_units)
  layers.append(rnn_cell.DropoutWrapper(cell, output_keep_prob=0.5))

  layers.append(OutputRNNCell(4, lstm1_units))
...
batch = tf.Variable(0)
optimizer = tf.train.AdagradOptimizer(learning_rate=0.01).minimize(loss, global_step=batch)
